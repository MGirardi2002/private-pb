...
# Inicializando a sessão e o cliente S3
session = boto3.Session(
    aws_access_key_id=aws_access_key_id,
    aws_secret_access_key=aws_secret_access_key,
    aws_session_token=aws_session_token,
...
# Função para executar consultas S3 Select
    ...
    response = s3.select_object_content(
        ...
        InputSerialization={
            "CSV": {
                ...
                "FieldDelimiter": delimiter,  # Especificando o delimitador correto
            }
        },
...
# Ajustando a consulta SQL para lidar com nomes de colunas contendo caracteres especiais
query = """
    SELECT 
...
# Executando a consulta e salvando os resultados
try:
    results = s3_select_query(bucket_name, file_key, query)

    # Salvar os resultados em um arquivo CSV, mantendo o delimitador ;
